...
export async function GET(
  ...
  try {
    ...
    if (!customer && email) {
      const customers =
        await customerModuleService.listCustomers({ email })
      ...
      return {
        ...
        items:
          order.items?.map((item: any) => {
            const enrichedUnitPrice = enrichPrice(
              item.unit_price || 0,
              currencyCode
            )
            const enrichedItemTotal = enrichPrice(
              item.total || 0,
              currencyCode
            )
        ...
        shipping_methods:
          order.shipping_methods?.map((m: any) => {
            const enrichedMethodAmount = enrichPrice(
              m.amount || 0,
              currencyCode
            )
            return {
              ...
              name: m.name,
              amount: m.amount,
              amount_display: enrichedMethodAmount.display_amount,
              amount_formatted: enrichedMethodAmount.formatted,
            }
          }) || [],
